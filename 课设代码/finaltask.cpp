...
{
    int toal_jdn=0, now_line = 1, now_jdn=0, line_btn=0;   //该行之前总节点个数，行号（不随每行结束清零）。该行节点个数，行字节（随每行结束清零）
    int i, j, filename_L;
    char *filename;     //用于存储文件名
    Sjjd p = head;

    filename = cat_out_filename(fname); //存储文件名
    ...
    printf("总单词数  本行单词数  当前行号  \n");    //每个信息占十个位宽，相邻信息间隔为两个空格
    do         //文件内容输出开始
    {
        if (*(p->Data) != '\n')     //判别换行符"\n"的处理
        {   //非换行符处理
            if (*(p->Data) == '\t')
            {
                //水平制表符处理（水平制表符形式上用连续的10个空格代替），也记为一个节点等同于其符号字符进行统计处理
                for (i = 1; i <= 10; i++)        //空格填充对齐
                {
                    putchar(' ');
                }
                toal_jdn++, now_jdn++, line_btn += p->da_lg;
            }
            else
            {   //普通字符处理(空格包含在内)，直接输出
                printf("%s", p->Data), toal_jdn++, now_jdn++, line_btn += p->da_lg;
            }
        }
        else
        {   //换行符处理（换行符形式上用连续的空格代替），但换行符记为一个节点等同于其符号字符进行统计处理
            toal_jdn++, now_jdn++;
        }

        if ( ((line_btn >= 45 && p->next != NULL) && (p->next->da_lg != 1 || *(p->next->Data) == '\n')) || *(p->Data) == '\n')   //行尾处理，用空格填充
        {
            for (i = 1; i < 60 - line_btn; i++)      //空格填充对齐
            {
                putchar(' ');
            }       printf("|");
            printf("%-10d  %-10d  %-10d\n", toal_jdn - now_jdn, now_jdn/2, now_line);  //每个信息占十个位宽，相邻信息间隔为两个空格
            now_line++, now_jdn = 0, line_btn = 0;
        }

        if (p->next == NULL)    //判断输出是否结束
        {
            for (i = 1; i < 60 - line_btn; i++)      //空格填充对齐
            {
                putchar(' ');
            }       printf("|");
            printf("%-10d  %-10d  %-10d\n", toal_jdn - now_jdn, now_jdn, now_line);  //每个信息占十个位宽，相邻信息间隔为两个空格
...
{
    char del[50], tmp;      //存放待删除字符串、tmp用于清除缓冲区
    Find_data fd_head, p_fd;    //定义查找结果链表相关指针
    int choice, i;

    printf("请输入要删除的字符串：");
    scanf("%s", del);
    while ((tmp = getchar()) != '\n' && tmp != EOF);    //清除输入缓冲区
    if (strcmp(del, "\\t") == 0)    //对制表符进行处理
        strcpy(del, "\t");
    else if (strcmp(del, "\\n") == 0)   //对回车进行处理
        strcpy(del, "\n");
    else if (strcmp(del, "\\s") == 0)   //对空格进行处理
        strcpy(del, " ");

    p_fd = fd_head = find(head, del);     //调用查找函数查找对应内容
    if (fd_head == NULL)    //要删除的字符串不存在处理
        return head;
    ...
    else    //删除指定的查找内容
    {
        ...
        {
            if (p_fd->fd_next == NULL)      //判断，输入的序号超过查找到的个数
            {
